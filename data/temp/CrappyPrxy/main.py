'''
This program is part of EternalBypass, an network of methods to
...
def fixURL(url):
    ...
    for script in soup.find_all("script"):
        ...
        if script.attrs.get("src"):
            ...
            if search_script not in script_files:
                ...
                try:
                    Path('static/'+search_script2).mkdir(parents=True, exist_ok=True)
                    #os.mkdir('static/'+search_script2)

                except Exception as er:
                    print("error creating static folder. "+str(er))
                    pass
                if url not in script_url:
                    ...
                    try:
                        ...
                        Path(search_script).mkdir(parents=True, exist_ok=True)
                        #os.mkdir(search_script)
                        os.system('rm -rf '+search_script)
                        f = open(search_script, 'w')
    ...
    for css in soup.find_all("link"):
        ...
        if css.attrs.get("href"):
            ...
            if search_css not in css_files:
                ...
                try:
                    Path('static/' + search_css2).mkdir(parents=True, exist_ok=True)
                ...
                if url not in css_url:
                    if '.ico' in css_url:
                        ...
                        try:
                            ...
                            Path(search_css).mkdir(parents=True, exist_ok=True)
                            # os.mkdir(search_script)
                            os.system('rm -rf ' + search_css)
                            f = open(search_css, 'w')
                        f.write(re)
                        f.flush()
                        f.close()
    ...
    #        if url not in img.get('src'):
        ...
        if search_img not in img_files:
            ...
            try:
                Path('static/' + search_img2).mkdir(parents=True, exist_ok=True)
            ...
            if url not in img_url:
                ...
                try:
                    ...
                    Path(search_img).mkdir(parents=True, exist_ok=True)
                    # os.mkdir(search_script)
                    os.system('rm -rf ' + search_img)
                    f = open(search_img, 'w')
                f.write(re.replace('\\n', '\n').replace("b'", '').strip("'"))
                f.flush()
                f.close()
    ...
    if debug == True:
       ...
       deb_f = open('debug.html', 'w')
       deb_f.write(append_html)
       deb_f.flush()
    return append_html
@app.route('/')
def getUrl():
        ...
        if 'list index out of range' in str(er):
            ...
            if '.favicon' not in ur:
                try:
                    ...
                    logfile = open('currentlog.log', 'a')
                    logfile.write('\nREQUESTING HTML FROM IN SITE URL: '+'http://'+ur)
                    logfile.flush()
                    logfile.close()
                    r4 = fixURL(ur)
                    ...
                    print('ERROR OCCURED TRYING TO FETCH '+ur+', SKIPPING.')
                    logfile = open('currentlog.log', 'a')
                    logfile.write('\nERROR OCCURED TRYING TO FETCH '+ur+', SKIPPING.')
                    logfile.flush()
                    logfile.close()
            else:
                pass
