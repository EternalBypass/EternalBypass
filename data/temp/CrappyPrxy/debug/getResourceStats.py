import psutil  #!!README!! A bug in numpy v.1.17.3 is causing an runtime error. Restart your console if this happens
...
process = subprocess.Popen([sys.executable, "testing/test0b.py"], cwd=parent)
pid1 = process.pid
active = check_pid(pid1)
if active == False:
    process.kill()
    print('ERROR: Procsess with PID '+str(pid1)+' was not active, cannot continue.')
    exit()
else:
    print('Proccess with PID '+str(pid1)+' is active, continuing!')
#pid1 = 6876
pid2 = pid1
#import time
#time.sleep(1)
