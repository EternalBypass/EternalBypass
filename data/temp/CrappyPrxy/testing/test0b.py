        ...
        if script.attrs.get("src"):
            ...
            if search_script not in script_files:
                ...
                if url not in script_url:
                    ...
                    e.flush()
                    e.close()
                    append_html = str(append_html).replace(url+search_script, search_script).replace('/'+search_script2, search_script2)
                script_fix += 1
        ...
        if css.attrs.get("href"):
            ...
            if search_css not in css_files:
                ...
                if url not in css_url:
                    ...
                    append_html = str(append_html).replace(url+search_css, search_css).replace('/'+search_script2, search_script2)
                css_fix += 1
...
#        if url not in img.get('src'):
        ...
        if search_img not in img_files:
            ...
            if url not in img_url:
                ...
                e.flush()
                e.close()
                append_html = str(append_html).replace(url+search_img, search_img).replace('/'+search_script2, search_script2)
            img_fix += 1
